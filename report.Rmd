...
Here we use a network-structured model to simulate disease spreading accross wards inside a healthcare facility and test the efficacy of intervention strategies on disease control. 
...
```{r, results='asis'}
cat('We consider a hospital facility with', 24, " wards. The wards are connected by health-care workers." )
```

Plots: 

1. connectivity plot for the newtork of wards

2. Population size (patients, HCWs)

### Epidemiology: pathogen (and immunity level)
We simulate the diffusion of **pathogen X** in a population with **no/low/high** immunity. 
...
```{r, results='asis'}
# cat('This report present synthetic results of', nsim,' simulations run for a', nward, "-wards healthcare facility. The wards are connected by the professionals. In this report, the simulations run with the baseline scenario: without any control nor surveillance measure (contact restriction, regular random screening, screening area at the admission, are compared to simulations run with:",  c("confinement/contact restriction to detected patients", "implementation of a screening area at patient admission including contact restriction/clinical examination/test", "screening of patients by implementation of tests on a random subset of patient population at regular intervals", "screening of healthcare workers by implementation of tests on a random subset of healthcare worker population at regular intervals", "positive probability to test symptomatic patients", "positive probability to test symptomatic healthcare workers")[c("ISO", "SA", "ScreenP", "ScreenH", "TestSympP", "TestSympH") %in% class(params$control_traj)] %>% paste(., collapse = " / "),".")
```
